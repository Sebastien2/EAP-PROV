...
static int tls_verify_cb(int preverify_ok, X509_STORE_CTX *x509_ctx)
{
	...
	//wpa_printf(MSG_INFO, "PERSO: tls_verify_cb writing in file ");

	FILE *fp;

   fp = fopen("/home/pi/radius/is_client_authenticated_in_eap_ttls", "w");
   fprintf(fp, "%d", preverify_ok);
   fclose(fp);

	//wpa_printf(MSG_INFO, "PERSO: tls_verify_cb returns %d ", preverify_ok);
	return preverify_ok;
}
...
{
	wpa_printf(MSG_INFO, " %s: %s called ", __FILE__, __FUNCTION__);
	...
	if (conn == NULL)
		return -1;
	wpa_printf(MSG_INFO, "PERSO: verify_peer=%d", verify_peer);
	if (verify_peer == 2) {
		conn->ca_cert_verify = 1;
