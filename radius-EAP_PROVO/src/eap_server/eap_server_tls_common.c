					 ...
					 int eap_type, int version, u8 id)
{
	...
	/* Write in req what is in position wpabuf_head_u8(data->tls_out) + data->tls_out_pos on length send_len */
	wpabuf_put_data(req, wpabuf_head_u8(data->tls_out) + data->tls_out_pos,
			send_len);
	data->tls_out_pos += send_len;
	//wpa_printf(MSG_INFO, "data->tls_out_pos=%d; send_len=%d", data->tls_out_pos, send_len);
