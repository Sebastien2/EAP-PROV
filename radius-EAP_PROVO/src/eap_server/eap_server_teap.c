...
struct eap_teap_data {
	...
	int wants_provisioning;
	u8 *provisioning_request;
	size_t provisioning_request_len;
};
...
static void eap_teap_state(struct eap_teap_data *data, int state)
{
	...
	data->state = state;
	wpa_printf(MSG_INFO, "\033[0;32m EAP-TEAP \033[0m: switching to state %s", eap_teap_state_txt(data->state));
}
...
static void * eap_teap_init(struct eap_sm *sm)
{
	...
	//TODO PERSO: modified the second argument to request a cleint certificate
	if (eap_server_tls_ssl_init(sm, &data->ssl,
				    /*sm->cfg->eap_teap_auth == 2 ? 2 : 0*/2,
				    EAP_TYPE_TEAP)) {
	...
	if (!data->srv_id) {
		eap_teap_reset(sm, data);
		wpa_printf(MSG_INFO, "PERSO: problem !data->srv_id");
		return NULL;
	}
	...
	if (!data->srv_id_info) {
		eap_teap_reset(sm, data);
		wpa_printf(MSG_INFO, "PERSO: problem !data->srv_id_info");
		return NULL;
	}
	...
	//TODO PERSO: start the provisioning
	data->wants_provisioning=0;
	data->provisioning_request=os_malloc(1000);
...
static void eap_teap_reset(struct eap_sm *sm, void *priv)
{
	...
	os_free(data->identity);
	os_free(data->provisioning_request); //PERSO attribute
...
static struct wpabuf * eap_teap_build_phase2_req(struct eap_sm *sm,
						 ...
						 u8 id)
{
	wpa_printf(MSG_INFO, "\033[0;32m EAP-TEAP \033[0m: buiding a phase 2 message");
	struct wpabuf *req, *final, *final2, *config_buf, *config_buf2, *id_tlv = NULL;
	...
	//wpa_printf(MSG_INFO, "PERSO: intermediary");
	if(id_tlv==NULL)
	{
		//wpa_printf(MSG_INFO, "\033[0;32m EAP-TEAP \033[0m: id_tlv is NULL");
	}
	else
	{
		wpa_hexdump(MSG_MSGDUMP, "PERSO: content of id_tlv buffers", id_tlv->buf, id_tlv->used);
	}
	
	final=wpabuf_concat(eap_teap_tlv_eap_payload(req), id_tlv);
...
// PERSO: contains the config&provisioning data
static struct wpabuf * eap_teap_build_crypto_binding(
	struct eap_sm *sm, struct eap_teap_data *data)
{
	wpa_printf(MSG_INFO, "\033[0;32m EAP-TEAP \033[0m building message containing provisioning and configuration data");
	struct wpabuf *buf, *buf2, *config_buf, *config_tlv_buf, *prov_buf, *prov_tlv_buf, *vendor_buf, *vendor_tlv_buf;
	struct teap_tlv_result *result;
	struct teap_tlv_crypto_binding *cb;
	u8 subtype, flags;

	int size_config=1000;
	int limit_size, limit_config_size=size_config+100, limit_prov_size=4000;
	limit_size=limit_config_size+limit_prov_size+500;
	buf = wpabuf_alloc(2 * sizeof(*result) + sizeof(*cb)+limit_size);
	if (!buf)
		return NULL;
	...
	/* Provisioning */
	if(data->wants_provisioning)
	{
		//we copy the csr in a file
		char *filename_csr="/home/pi/sebastien/temp/cert.csr.der";
		if(true)
		{
			/* wpa_printf(MSG_DEBUG, "\n\nOpening message file\n\n"); */
			FILE *file = fopen(filename_csr, "w");
			for(int i=0;i<data->provisioning_request_len;i++)
			{
				fputc(data->provisioning_request[i], file);
			}
			fclose(file);
		}
		else
		{
			wpa_printf(MSG_ERROR, "PERSO: writing certificate request to file failed, stopped provisioning");
		}
		char * command_convert_generate = os_calloc(512, sizeof(char));
		int command_res=os_snprintf(command_convert_generate, 256, "openssl req -inform der -outform pem -in /home/pi/sebastien/temp/cert.csr.der -out /home/pi/sebastien/temp/cert.csr.pem");
		system(command_convert_generate);
		os_free(command_convert_generate);
		
		char * command_generate = os_calloc(512, sizeof(char));
		command_res=os_snprintf(command_generate, 256, "openssl x509 -req -in /home/pi/sebastien/temp/cert.csr.pem -CA /etc/hostapd/cert.pem -CAkey /etc/hostapd/key.pem -CAcreateserial -out /home/pi/sebastien/temp/test.cert");
		system(command_generate);
		os_free(command_generate);
		//TODO: PERSO send the certificate
		
		//wpa_printf(MSG_INFO, "PERSO: creation of the certificate done %d", command_res);

		char * command_reconvert_generate = os_calloc(512, sizeof(char));
		command_res=os_snprintf(command_reconvert_generate, 256, "openssl crl2pkcs7 -nocrl -in /home/pi/sebastien/temp/test.cert -out /home/pi/sebastien/temp/test.cert.p7b -certfile /etc/hostapd/cert.pem");
		system(command_reconvert_generate);
		os_free(command_reconvert_generate);

		//We read the file and create the TLV
		prov_buf=wpabuf_alloc(limit_prov_size);
		prov_tlv_buf=wpabuf_alloc(limit_prov_size+20);
		char pkcs7_file_path[]="/home/pi/sebastien/temp/test.cert.p7b";
		if(access( pkcs7_file_path, F_OK ) == 0  && access( pkcs7_file_path, R_OK) == 0)
		{
			FILE *file = fopen(pkcs7_file_path, "r");
			fseek(file, 0L, SEEK_END);
			size_t file_size = ftell(file);
			rewind(file);
			if(file_size > limit_prov_size)
			{
				fclose(file);
				wpa_printf(MSG_ERROR, "PERSO: The pkcs7 file is too big!\n\n\n");
			}
			else
			{
				//wpa_printf(MSG_DEBUG, "PERSO: Reading the pkcs7 file");
				// add couple of extra bytes to stay safe 
				
				fread(prov_buf->buf, file_size, sizeof(char), file);
				fclose(file);
				
				prov_buf->used = file_size;
				//wpa_printf(MSG_DEBUG, "PERSO: %zu byte is read", file_size);
				//wpa_hexdump_ascii(MSG_DEBUG, "PERSO: The read content", prov_buf->buf, file_size);
				
				//wpa_printf(MSG_DEBUG, "PERSO: Size of file being sent: %d", file_size);
				eap_teap_put_tlv_buf(prov_tlv_buf, TEAP_TLV_PKCS7, prov_buf); //202: type of provisioning data
				//wpa_hexdump_ascii(MSG_DEBUG, "PERSO: The read content", prov_tlv_buf->buf, prov_tlv_buf->used);
				wpa_hexdump_ascii(MSG_INFO, "\033[0;32m EAP-TEAP \033[0m Content of the PKCS#7 TLV containing the provisioning data", prov_tlv_buf->buf, prov_tlv_buf->used);
				buf2=wpabuf_concat(buf, prov_tlv_buf);
				buf=wpabuf_alloc(limit_size);
				for(int i=0; i<buf2->used;i++)
				{
					buf->buf[i]=buf2->buf[i];
				}
				buf->used=buf2->used;
				wpabuf_free(buf2);
			}
		...
		{
			wpa_printf(MSG_ERROR, "PERSO: File test.cert.p7b could not be opened\n\n\n");
		}
	}
	...
	/*  Vendor-Specific TLV: config data */
	int vendor_specific=1;
	if(vendor_specific)
	{
		//wpa_printf(MSG_INFO, "PERSO: vendor-specific TLV construction");
		int provisioning_success=0, configuration_success=0;

		vendor_buf=wpabuf_alloc(limit_config_size+limit_prov_size);
		vendor_tlv_buf=wpabuf_alloc(limit_config_size+limit_prov_size);
		
		//TODO: add config data in it
		config_buf=wpabuf_alloc(limit_config_size);
		config_tlv_buf=wpabuf_alloc(limit_config_size+20);
		//we put the configuration data inside
		char configuration_file_path[]="/home/pi/radius/notification-message/example_configuration_file.json";
		
		if(access( configuration_file_path, F_OK ) == 0  && access( configuration_file_path, R_OK) == 0)
		{
			FILE *file = fopen(configuration_file_path, "r");
			fseek(file, 0L, SEEK_END);
			size_t file_size = ftell(file);
			rewind(file);
			if(file_size > limit_prov_size)
			{
				fclose(file);
				wpa_printf(MSG_ERROR, "PERSO: The \"message.json\" file is too big!\n\n\n");
			}
			else
			{
				//wpa_printf(MSG_DEBUG, "PERSO: Reading the config message from \"message.json\" file");
				// add couple of extra bytes to stay safe 
				/*
				for(int i=0;i<size_config;i++)
				{
					wpabuf_put_u8(config_buf, 6);
				}
				*/
				
				fread(config_buf->buf, file_size, sizeof(char), file);
				fclose(file);
				
				config_buf->used = file_size;
				//wpa_printf(MSG_DEBUG, "PERSO: %zu byte is read", file_size);
				//wpa_hexdump_ascii(MSG_DEBUG, "PERSO: The read content", config_buf->buf, file_size);
				
				
				//wpa_printf(MSG_DEBUG, "PERSO: Size of file being sent: %d", file_size);
				eap_teap_put_tlv_buf(config_tlv_buf, 201, config_buf);  // type 201: configuration data
				
				wpa_hexdump_ascii(MSG_INFO, "\033[0;32m EAP-TEAP \033[0m: Content of the TLV Vendor-Specific built: ", config_tlv_buf->buf, config_tlv_buf->used);
				configuration_success=1;
			}
		}
		else
		{
			wpa_printf(MSG_ERROR, "PERSO: File could not be opened\n\n\n");
		}
		


		wpa_hexdump(MSG_MSGDUMP, "PERSO: content of buffers config_tlv_buf", config_tlv_buf->buf, config_tlv_buf->used);

		//This sends a certificate in a vendor-specific TLV. This is not needed as we use the PKCS7 TLV according to the EAP-TEAP specification.
		if(false)
		{
			// TODO PERSO: provisioning
			prov_buf=wpabuf_alloc(limit_prov_size);
			prov_tlv_buf=wpabuf_alloc(limit_prov_size+20);
			char provisioning_file_path[]="/home/pi/radius/notification-message/example_provisioning_file.txt";
			if(access( provisioning_file_path, F_OK ) == 0  && access( provisioning_file_path, R_OK) == 0)
			{
				FILE *file = fopen(provisioning_file_path, "r");
				fseek(file, 0L, SEEK_END);
				size_t file_size = ftell(file);
				rewind(file);
				if(file_size > limit_prov_size)
				...
				else
				{
					...
					wpa_printf(MSG_DEBUG, "PERSO: Size of file being sent: %d", file_size);
					eap_teap_put_tlv_buf(prov_tlv_buf, 202, prov_buf); //202: type of provisioning data
					wpa_hexdump_ascii(MSG_DEBUG, "PERSO: The read content", prov_tlv_buf->buf, prov_tlv_buf->used);
		...
		if(provisioning_success && configuration_success)
		{
			vendor_buf=wpabuf_concat(config_tlv_buf, prov_tlv_buf);
		}
		else if(configuration_success)
		{
			vendor_buf=config_tlv_buf;
		}
		else if(provisioning_success)
		{
			vendor_buf=prov_tlv_buf;
		}
		else
		{
			vendor_buf=wpabuf_alloc(1);
		}
		
		for(int i=0;i<4;i++)
		{
			wpabuf_put_u8(vendor_tlv_buf, 9);
		}
		vendor_buf=wpabuf_concat(vendor_tlv_buf, vendor_buf);
		wpa_hexdump_ascii(MSG_DEBUG, "PERSO: The read content of vendor_buf", vendor_buf->buf, vendor_buf->used);
		
		wpabuf_free(config_buf);
		wpabuf_free(prov_buf);

		vendor_tlv_buf=wpabuf_alloc(limit_config_size+limit_prov_size);
		eap_teap_put_tlv_buf(vendor_tlv_buf, TEAP_TLV_VENDOR_SPECIFIC, vendor_buf);
		//pa_printf(MSG_INFO, "PERSO: size buf: (%d, %d), size vendor_tlv_buf: (%d, %d)", buf->used, buf->size, vendor_tlv_buf->used, vendor_tlv_buf->size);
		buf2=wpabuf_concat(buf, vendor_tlv_buf);
		...
		//wpa_printf(MSG_INFO, "PERSO: ...done");
	}
	...
	wpa_hexdump(MSG_MSGDUMP, "PERSO: Intermediary MSK Compound MAC", cb->msk_compound_mac, sizeof(cb->msk_compound_mac));
	wpa_hexdump(MSG_MSGDUMP, "PERSO: Intermediary EMSK Compound MAC", cb->emsk_compound_mac, sizeof(cb->emsk_compound_mac));
...
static struct wpabuf * eap_teap_build_pac(struct eap_sm *sm,
					  struct eap_teap_data *data)
{
	wpa_printf(MSG_INFO, "\033[0;32m EAP-TEAP \033[0m: building PAC");
...
{
	//wpa_printf(MSG_INFO, "PERSO: %s", __FUNCTION__);
	struct eap_teap_tlv_parse tlv;
	...
	//TODO PERSO add the cert request to the data
	if(tlv.cert_request)
	{
		wpa_printf(MSG_INFO, "\033[0;32m EAP-TEAP \033[0m: reception of PKC10 TLV with a certificate request");
		for(int i=0;i<tlv.cert_request_len;i++)
		{
			data->provisioning_request[i]=tlv.cert_request[i];
		}
		data->provisioning_request_len=tlv.cert_request_len;
		data->wants_provisioning=1;
		wpa_hexdump_ascii(MSG_MSGDUMP, "PERSO: CSR: Received", data->provisioning_request, data->provisioning_request_len);
	}
	//done
...
{
	wpa_printf(MSG_INFO, "\033[0;32m EAP-TEAP \033[0m: processing a phase 2 message");
...
static int eap_teap_process_phase1(struct eap_sm *sm,
				   struct eap_teap_data *data)
{
	wpa_printf(MSG_INFO, "\033[0;32m EAP-TEAP \033[0m: processing a phase 1 message");
...
static void eap_teap_process(struct eap_sm *sm, void *priv,
			     struct wpabuf *respData)
{
	wpa_printf(MSG_INFO, "\033[0;32m EAP-TEAP \033[0m: processing a message");
	struct eap_teap_data *data = priv;
	const u8 *pos;
	size_t len;
...
int eap_server_teap_register(void)
{
	...
	//wpa_printf(MSG_INFO, "\n\n PERSO: %s : eap_server_teap_register() called  \n\n", __FILE__);
