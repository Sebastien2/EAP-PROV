			       ...
			       const char *buf, char *pos, int line)
{
	...
	if (os_strcmp(buf, "interface") == 0) {
		...
		//wpa_printf(MSG_INFO, "PERSO: filling in pac_opaque_encr_key");
		...
		//wpa_printf(MSG_INFO, "PERSO; reading eap_fast_a_id");
		...
		if (bss->eap_fast_a_id == NULL ||
			...
			//wpa_printf(MSG_INFO, "PERSO: successful eap_fast_a_id provisioning");
			bss->eap_fast_a_id_len = idlen / 2;
		}
	} else if (os_strcmp(buf, "eap_fast_a_id_info") == 0) {
		os_free(bss->eap_fast_a_id_info);
		//wpa_printf(MSG_INFO, "PERSO: provisioning of eap_fast_a_id_info");
