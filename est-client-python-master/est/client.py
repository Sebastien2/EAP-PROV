...
class Client(object):
    ...
    def simpleenroll(self, csr):
        ...
        if self.jwt is None:
            ...
            headers = {'Content-Type': 'application/pkcs10', 'Authorization': 'Bearer ' + self.jwt}
        content = est.request.post(url, csr, auth=auth if self.jwt is None else None, headers=headers,
            verify=self.implicit_trust_anchor_cert_path)
        pem = self.pkcs7_to_pem(content)
    ...
    def set_jwt(self, jwt):
        """Set up JWT authentication header
        Args:
            jwt(str)
        """
        self.jwt = jwt
