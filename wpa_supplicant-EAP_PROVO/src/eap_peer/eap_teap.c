...
#define EAP_TEAP_PROV_AUTH 2
	...
	int has_requested_provisioning;
};
...
{
	const char *pos;
	wpa_printf(MSG_INFO, "PERSO: %s %s", __FUNCTION__, phase1);
...
static void * eap_teap_init(struct eap_sm *sm)
{
	...
	wpa_printf(MSG_INFO, "PERSO: initialization %s", config->phase1);
	if (config->phase1)
	...
	if ((data->provisioning_allowed & EAP_TEAP_PROV_AUTH) &&
		...
		wpa_printf(MSG_INFO, "PERSO: provisioning with pac: %d", data->provisioning_allowed);
	}
	else
	{
		wpa_printf(MSG_INFO, "PERSO: provisioning with pac prohibited %d %d", data->provisioning_allowed, EAP_TEAP_PROV_AUTH);
		wpa_printf(MSG_INFO, "PERSO: config details: %s %s", config->cert.ca_cert, config->cert.ca_path);
	}
	...
	//TODO PERSO; provisioning parameters
	data->wants_provisioning=1;
	data->has_requested_provisioning=0;
				   ...
				   struct wpabuf **resp)
{
	...
	wpa_printf(MSG_ERROR, "\033[0;32m EAP-TEAP \033[0m processing phase 2 message");
	...
	enum teap_identity_types req_id_type)
{
	...
	wpa_printf(MSG_INFO, "\033[0;32m EAP-TEAP \033[0m: building the provisioning request");
	//TODO PERSO: add PKCS10 TLV for certificate request
	if(data->wants_provisioning && !data->has_requested_provisioning)
	{
		struct wpabuf *pkcs10_buf, *pkcs10_tlv_buf;
		//int max_size_pkcs10=1000;
		//pkcs10_buf=wpabuf_alloc(max_size_pkcs10);
		//pkcs10_tlv_buf=wpabuf_alloc(max_size_pkcs10+20);
		

		char * csr_path="/home/pi/radius_creds/req.der";
		if(access( csr_path, F_OK ) == 0  && access( csr_path, R_OK) == 0)
		{
			/* wpa_printf(MSG_DEBUG, "\n\nOpening message file\n\n"); */
			FILE *file = fopen(csr_path, "r");
			
			fseek(file, 0L, SEEK_END);
			size_t file_size = ftell(file);
			rewind(file);
			
			wpa_printf(MSG_DEBUG, "PERSO: Reading the csr file");
			/* add couple of extra bytes to stay safe */
			pkcs10_buf = wpabuf_alloc(file_size + 5);
			pkcs10_tlv_buf=wpabuf_alloc(file_size+20);
			
			fread(pkcs10_buf->buf, file_size, sizeof(char), file);
			fclose(file);
				
			pkcs10_buf->used = file_size;
			//wpa_printf(MSG_DEBUG, "PERSO: %zu byte is read", file_size);
			//wpa_hexdump_ascii(MSG_DEBUG, "PERSO: The read content", pkcs10_buf->buf, file_size);
			
			eap_teap_put_tlv_buf(pkcs10_tlv_buf, TEAP_TLV_PKCS10, pkcs10_buf);
			resp=wpabuf_concat(resp, pkcs10_tlv_buf);
			//wpa_hexdump_ascii(MSG_INFO, "PERSO: The read content of the buffer", resp->buf, resp->used);
		}
		else
		{
			wpa_printf(MSG_ERROR, "\n\n\nPERSO: failure to read the CSR and put it in a TLV, provisioning request abandoned");
		}
		data->has_requested_provisioning=1;
	}
				 ...
				 const struct teap_tlv_crypto_binding *cb)
{
	...
	if (cb->nonce[EAP_TEAP_NONCE_LEN - 1] & 0x01) {
		wpa_printf(MSG_INFO, "EAP-TEAP: Invalid Crypto-Binding TLV Nonce in request");
		return -1;
	}
				      ...
				      struct wpabuf **out_data)
{
	...
	//TODO: PERSO obtaining the PKCS7
	if(tlv.pkcs7)
	{	
		wpa_printf(MSG_ERROR, "\033[0;32m EAP-TEAP \033[0m receiving the PKCS#7 TLV for provisioning");
		//wpa_printf(MSG_INFO, "PERSO: copying PKCS7 to file");
		//we write it into a file
		char * pkcs7_file_path="/home/pi/radius_creds/test.cert.p7b";
		FILE *file = fopen(pkcs7_file_path, "w");
		for(int i=0;i<tlv.pkcs7_len;i++)
		{
			fputc(tlv.pkcs7[i], file);
		}
		fclose(file);
	}
	...
	if(tlv.vendor_specific)
	{
		wpa_printf(MSG_ERROR, "\033[0;32m EAP-TEAP \033[0m receiving the vendor-specific TLV for configuration");
		wpa_hexdump_ascii(MSG_ERROR, "Configuration data: ", tlv.vendor_specific, tlv.vendor_specific_len);
	}
	...
	if(data->current_pac)
	{
		wpa_printf(MSG_INFO, "PERSO: already current_pac");
	}
	if(!data->provisioning)
	{
		wpa_printf(MSG_INFO, "PERSO: no provisioning");
	}
	if(!data->anon_provisioning)
	{
		wpa_printf(MSG_INFO, "PERSO: already data->anon_provisioning");
	}
	if(failed)
	{
		wpa_printf(MSG_INFO, "PERSO: failed");
	}
	if(tlv.pac)
	{
		wpa_printf(MSG_INFO, "PERSO: already tlv.pac");
	}
	if(!tlv.crypto_binding)
	{
		wpa_printf(MSG_INFO, "PERSO: no tlv.crypto_binding");
	}
	if(!eap_teap_allowed_anon_prov_cipher_suite(data->tls_cs))
	{
		wpa_printf(MSG_INFO, "PERSO: no eap_teap_allowed_anon_prov_cipher_suite(data->tls_cs)");
	}
	*/
	//TODO PERSO: remove those 2 lines, they should only be inside the tests
	...
	if (!data->current_pac && data->provisioning && !failed && !tlv.pac &&
		 ...
		 * provisioning.
		 */
		wpa_printf(MSG_ERROR, "\033[0;32m EAP-TEAP \033[0m sending a certificate request in PKCS#10 TLV");
				  ...
				  const u8 *pos, size_t left)
{
	...
	if (data->resuming && data->current_pac) {
		wpa_printf(MSG_INFO, "PERSO: case 1");
		wpa_printf(MSG_DEBUG,
		...
		wpa_printf(MSG_INFO, "PERSO: case 2");
		/*
		...
		{
			wpa_printf(MSG_INFO, "PERSO: problem in using current PAC");
			return -1;
		}
		...
		if (eap_teap_clear_pac_opaque_ext(sm, data) < 0)
			return -1;
		wpa_printf(MSG_INFO, "PERSO: data provisioning set to 1");
		data->provisioning = 1;
	}
					...
					const struct wpabuf *reqData)
{
	...
	wpa_printf(MSG_INFO, "\033[0;32m EAP-TEAP \033[0m processing message");
	...
	if (tls_connection_established(sm->ssl_ctx, data->ssl.conn) &&
		...
		wpa_printf(MSG_INFO, "	processing phase 2 encrypted message");
		...
		wpa_printf(MSG_INFO, "	processing TLS handshake");
		...
		if (sm->waiting_ext_cert_check) {
			wpa_printf(MSG_DEBUG, "EAP-TEAP: Waiting external server certificate validation");
			wpabuf_free(data->pending_resp);
