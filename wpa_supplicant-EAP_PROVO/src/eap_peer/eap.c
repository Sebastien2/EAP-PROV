...
static void eap_sm_processIdentity(struct eap_sm *sm, const struct wpabuf *req)
{
	...
	//wpa_printf(MSG_INFO, "PERSO %s postion marker", __FUNCTION__);
...
struct wpabuf * eap_sm_buildIdentity(struct eap_sm *sm, int id, int encrypted)
{
	...
	if (sm->m && sm->m->get_identity &&
		...
		wpa_printf(MSG_INFO, "PERSO: message identity with real identity");
	...
	if (config->pcsc) {
#ifdef PCSC_FUNCS
	wpa_printf(MSG_INFO, "PCSC_FUNCS bloc entered");
		if (!identity) {
			wpa_printf(MSG_INFO, "PCSC_FUNCS bloc error 1");
			if (eap_sm_get_scard_identity(sm, config) < 0)
			{
				wpa_printf(MSG_INFO, "PCSC_FUNCS bloc error 2");
				return NULL;
			}
			identity = config->identity;
			identity_len = config->identity_len;
			wpa_printf(MSG_INFO, "PCSC_FUNCS bloc step 2");
			wpa_hexdump_ascii(MSG_DEBUG,
					  "permanent identity from IMSI",
					  identity, identity_len);
		} else if (eap_sm_set_scard_pin(sm, config) < 0) {
			wpa_printf(MSG_INFO, "PCSC_FUNCS bloc error 3");
			return NULL;
		}
#else /* PCSC_FUNCS */
		return NULL;
