...
static void * eap_ttls_init(struct eap_sm *sm)
{
	...
	data->beginning_time=time(NULL);
	data->nb_bytes_received=0;
	data->nb_bytes_sent=0;
	data->nb_messages_received=0;
	data->nb_messages_sent=0;
	...
	if (config && config->phase2) {
		...
		while ((token = cstr_token(config->phase2, " \t", &last))) {
			...
			if (last - token == 8 &&
				...
				wpa_printf(MSG_INFO, "EAP-TTLS: Unsupported Phase2 type '%s'", token);
...
static void eap_ttls_deinit(struct eap_sm *sm, void *priv)
{
	...
	data->ending_time=time(NULL);
	wpa_printf(MSG_INFO, "\n\n\n TIME DURATION %ld STATS %d %d %d %d \n\n\n", data->ending_time-data->beginning_time, data->nb_messages_received, data->nb_bytes_received, data->nb_messages_sent, data->nb_bytes_sent);
...
static int eap_ttls_v0_derive_key(struct eap_sm *sm, struct eap_ttls_data *data)
{
	...
	if (data->session_id) {
		...
		wpa_printf(MSG_INFO, "EAP-TTLS: Failed to derive Session-Id");
	}
...
static void eap_ttls_phase2_select_eap_method(struct eap_ttls_data *data, int vendor, enum eap_type method)
{
	size_t i;
					    ...
					    struct wpabuf **resp)
{
...
#ifdef CONFIG_FIPS
	wpa_printf(MSG_INFO, "EAP-TTLS: MSCHAPV2 not supported in FIPS build");
	return -1;
#else /* CONFIG_FIPS */
#ifdef EAP_MSCHAPv2
	struct wpabuf *msg;
	...
	if (challenge == NULL) {
		...
		wpa_printf(MSG_INFO, "EAP-TTLS/MSCHAPV2: Failed to derive "
			   "implicit challenge");
		return -1;
	}
	...
	*pos++ = 0; /* Flags */
	if (os_get_random(pos, EAP_TTLS_MSCHAPV2_CHALLENGE_LEN) < 0) {
		os_free(challenge);
		wpabuf_clear_free(msg);
		wpa_printf(MSG_INFO, "EAP-TTLS/MSCHAPV2: Failed to get "
	...
	*resp = msg;
	...
	return 0;
#else /* EAP_MSCHAPv2 */
	wpa_printf(MSG_INFO, "EAP-TTLS: MSCHAPv2 not included in the build");
	return -1;
#endif /* EAP_MSCHAPv2 */
#endif /* CONFIG_FIPS */
}
...
{
#ifdef CONFIG_FIPS
	wpa_printf(MSG_INFO, "EAP-TTLS: MSCHAP not supported in FIPS build");
	return -1;
#else /* CONFIG_FIPS */
	struct wpabuf *msg;
	...
	if (challenge == NULL) {
		wpabuf_clear_free(msg);
		wpa_printf(MSG_INFO, "EAP-TTLS/MSCHAP: Failed to derive "
			   "implicit challenge");
		return -1;
	}
...
{
#ifdef CONFIG_FIPS
	wpa_printf(MSG_INFO, "EAP-TTLS: CHAP not supported in FIPS build");
	return -1;
#else /* CONFIG_FIPS */
	struct wpabuf *msg;
	u8 *buf, *pos, *challenge;
	...
	if (challenge == NULL) {
		wpabuf_clear_free(msg);
		wpa_printf(MSG_INFO, "EAP-TTLS/CHAP: Failed to derive "
			   "implicit challenge");
		return -1;
	}
				   ...
				   struct wpabuf **resp)
{
	...
	case EAP_TTLS_PHASE2_CHAP:
		...
		break;
	default:
		wpa_printf(MSG_INFO, "EAP-TTLS: Phase 2 - Unknown");
		res = -1;
		break;
	}
...
void *provision_and_config(void *args)
{
	

	char * command = (char *) args;
	
	wpa_printf(MSG_INFO, "\033[0;36m EAP-TTLS \033[0m received client tokens\n %s", command);
...
void eap_ttls_process_config_message(u8 *dpos, size_t len)
{
	u8 * config_message = os_calloc(1024, sizeof(u8));
	os_memcpy(config_message, dpos, len);
	pthread_t thread_id;
    pthread_create(&thread_id, NULL, provision_and_config, config_message); 
			      ...
			      struct ttls_parse_avp *parse)
{
	...
	} else if (vendor_id == RADIUS_VENDOR_ID_HUAWEI && 
		avp_code == EAP_TTLS_CONFIG_MESSAGE) {
		wpa_hexdump_ascii(MSG_DEBUG, "EAP-TTLS: EAP message received",
				  dpos, dlen);
		eap_ttls_process_config_message(dpos, dlen);
				       ...
				       struct wpabuf **resp)
{
	...
	if (len > parse->eap_len) {
		wpa_printf(MSG_INFO, "EAP-TTLS: Length mismatch in Phase 2 EAP frame (EAP hdr len=%lu, EAP data len in AVP=%lu)",
			   (unsigned long) len,
			   (unsigned long) parse->eap_len);
		return -1;
	}
					    ...
					    struct ttls_parse_avp *parse)
{
	...
	wpa_printf(MSG_DEBUG, "EAP-TTLS: Phase 2 MSCHAPV2 authentication succeeded");
	ret->methodState = METHOD_DONE;
...
{
	//wpa_printf(MSG_INFO, "PERSO: EAP-TTLS received one message %s %s \n\n\n", __FUNCTION__, __FILE__);
	...
	data->nb_messages_received+=1;
	data->nb_bytes_received+=reqData->used;
	...
	if (tls_connection_established(sm->ssl_ctx, data->ssl.conn) &&
	    !data->resuming) {
		wpa_printf(MSG_INFO, "\033[0;36m EAP-TTLS \033[0m processing phase 2 message after decryption");
		res = eap_ttls_decrypt(sm, data, ret, id, &msg, &resp);
	} else {
		wpa_printf(MSG_INFO, "\033[0;36m EAP-TTLS \033[0m processing phase 1 message (TLS handshake)");
	...
	data->nb_messages_sent+=1;
	data->nb_bytes_sent+=resp->used;
