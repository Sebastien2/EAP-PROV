		       ...
		       int tlv_type, u8 *pos, size_t len)
{
	...
	case TEAP_TLV_BASIC_PASSWORD_AUTH_RESP:
		...
		break;
	case TEAP_TLV_PKCS10: //TODO PERSO: add reading of PKCS10 for certificate request
		wpa_hexdump_ascii(MSG_MSGDUMP,
				  "EAP-TEAP: PKCS10 TLV",
				  pos, len);
		tlv->cert_request=pos;
		tlv->cert_request_len=len;
		break;
	case TEAP_TLV_PKCS7:
		wpa_hexdump_ascii(MSG_MSGDUMP,
				  "EAP-TEAP: PKCS7 TLV",
	...
	case TEAP_TLV_VENDOR_SPECIFIC:
		wpa_hexdump_ascii(MSG_MSGDUMP,
				  "EAP-TEAP: VENDOR-SPECIFIC TLV",
				  pos, len);
		tlv->vendor_specific=pos;
		tlv->vendor_specific_len=len;
		break;
	default:
		/* Unknown TLV */
		return -1;
